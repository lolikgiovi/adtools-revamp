class Base64Tools extends BaseTool {
  ...
  buildFileCard(data, actions = {}) {
    ...
    if (variant === "image") {
      infoInner = /*html*/ `
        <div class="file-card-info image-card">
          ...
          <div class="file-card-details">
            ...
            <p class="file-card-size">${window.Base64ToolsService.formatFileSize(size)}</p>
            ${
              dimensions
      ...
      const fileTypeIcon = window.Base64ToolsConstants.getFileTypeIcon(type);
      infoInner = /*html*/ `
        <div class="file-card-info binary-card">
          ...
          <div class="file-card-details">
            <p class="file-card-name" title="${name}">${name}</p>
            <p class="file-card-size">${window.Base64ToolsService.formatFileSize(size)}</p>
            <p class="file-card-meta"><span class="file-type">${window.Base64ToolsConstants.getFileTypeLabel(type)}</span></p>
          </div>
        </div>`;
    } else {
      // text/default
      const fileTypeIcon = window.Base64ToolsConstants.getFileTypeIcon("text/plain");
      infoInner = /*html*/ `
  ...
  showFileInfo(file, mode) {
    ...
    if (infoDiv) {
      infoDiv.innerHTML = /*html*/ `
        ...
        <div class="file-info-item">
          <strong>Size:</strong> ${window.Base64ToolsService.formatFileSize(file.size)}
        </div>
        <div class="file-info-item">
