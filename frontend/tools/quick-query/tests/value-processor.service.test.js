...
describe("ValueProcessorService", () => {
  ...
  describe("validateNumberPrecision", () => {
    ...
    it("should throw error when total digits exceed precision", () => {
      expect(() => service.validateNumberPrecision(1234.56, 5, 2, "test_field", "test_table")).toThrow(/exceeds maximum precision/);
      expect(UsageTracker.trackEvent).toHaveBeenCalledWith(
        "quick-query",
        "value_error",
        expect.objectContaining({ type: "precision_exceeded", table_name: "test_table" })
      );
    });

    it("should throw error when decimal digits exceed scale", () => {
      expect(() => service.validateNumberPrecision(123.456, 6, 2, "test_field", "test_table")).toThrow(/exceeds maximum scale/);
      expect(UsageTracker.trackEvent).toHaveBeenCalledWith(
        "quick-query",
        "value_error",
        expect.objectContaining({ type: "scale_exceeded", table_name: "test_table" })
      );
    });

    it("should throw error when integer digits exceed precision - scale", () => {
      expect(() => service.validateNumberPrecision(1234.5, 5, 2, "test_field", "test_table")).toThrow(
        /Integer part .* exceeds maximum allowed digits/
      );
      expect(UsageTracker.trackEvent).toHaveBeenCalledWith(
        "quick-query",
        "value_error",
        expect.objectContaining({ type: "integer_digits_exceeded", table_name: "test_table" })
      );
    });
  });
